...
@Service
@Log4j2
public class AppSendTextDataHandler implements IDataService {
    ...
    public RemoteTypeEnum getCode() {
        return RemoteTypeEnum.QW_APP_NOTICE_TEXT_MSG;
    }

    @Override
    public void buildRequest(Context context) {
        QwAppSendTextMsgReqDTO textMsgReqDTO = (QwAppSendTextMsgReqDTO) context.getQueryDTO();
    ...
    public QwAppSendMsgRespDTO buildResponse(String resp) {
        log.info("[{} response >>>>> {}]", RemoteTypeEnum.QW_APP_NOTICE_TEXT_MSG.name(), resp);
